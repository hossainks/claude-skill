...
test.describe("Booking Flow", () => {
  ...
  test("TC-001: Book a single ticket on a static event and see confirmation card", async ({
    page,
  }) => {
    // -- Step 1: Login --
    await login(page);
  ...
  test("TC-002: Book 3 tickets using the quantity stepper and verify confirmation card", async ({
    page,
  }) => {
    // -- Step 1: Login --
    await login(page);
    ...
    await page
      ...
      .click(); // Fix #1: no hardcoded ID
    await expect(
      page.getByRole("heading", { name: "Dilli Diwali Mela" }),
    ).toBeVisible();
  ...
  test("TC-003: Bookings list shows cards with confirmed status badge and View Details link", async ({
    page,
  }) => {
    ...
    await page
      ...
      .click(); // Fix #1: no hardcoded ID
    await expect(
      page.getByRole("heading", { name: /Hollywood Monsoon Night/i }),
    ).toBeVisible();
